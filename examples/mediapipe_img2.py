...
def render_skeleton_from_image(
    ...
    if preserve_bg:
        ...
        out = np.zeros_like(image)   # background nero

    POSE_COLOR = (0, 255, 0)      # verde
    HAND_COLOR = (255, 0, 0)      # blu (BGR)
    FACE_COLOR = (0, 200, 255)    # giallo-ish

    # ---- Pose (MULTI, stesso colore)
    if pose_res.pose_landmarks:
        for lm in pose_res.pose_landmarks:
            pts = [_to_px(p.x, p.y, w, h) for p in lm]
            draw_edges(out, pts, POSE33_EDGES,
                       color=POSE_COLOR, thickness=thickness)

    # ---- Hands (MULTI, stesso colore)
    if hand_res.hand_landmarks:
        for hand_lm in hand_res.hand_landmarks:
            pts = [_to_px(p.x, p.y, w, h) for p in hand_lm]
            draw_edges(out, pts, HAND21_EDGES,
                       color=HAND_COLOR, thickness=thickness)

    # ---- Face (MULTI, stesso colore)
    if face_res.face_landmarks:
        for face_lm in face_res.face_landmarks:
            pts = [_to_px(p.x, p.y, w, h) for p in face_lm]
            for conn_set in FACE_CONNECTIONS:
                draw_face_points(out, pts, conn_set,
                                 radius=point_radius, color=FACE_COLOR)
...
if __name__ == "__main__":
    ...
    if image is None:
        raise FileNotFoundError(
            f"Immagine non trovata o non leggibile: {image_path}")

    out = render_skeleton_from_image(
        image=image,
        models=models,
        thickness=2,
        point_radius=2,
