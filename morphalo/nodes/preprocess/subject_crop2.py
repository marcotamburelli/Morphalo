...
SUBJECT_CROP2_POSE_TARGETS: frozenset[str] = frozenset({
    'arms',
    'left-arm',
    'right-arm',
    'legs',
...
class PoseContext:
    """
    MediaPipe pose information used by SubjectCrop2.
...
def _build_limb_partition_context(
    ...
    *,
    ...
    if x1 < 0 or y1 < 0 or x2 > ctx.width or y2 > ctx.height or x2 <= x1 or y2 <= y1:
        raise RuntimeError(
            f"SubjectCrop2 node '{ctx.node_id}': invalid limb crop bbox "
...
def _segment_limb_target(
    ...
    *,
    ...
    if not all_geometries:
        raise RuntimeError(
            f"SubjectCrop2 node '{ctx.node_id}': could not derive "
    ...
    if not np.any(mask):
        raise RuntimeError(
            f"SubjectCrop2 node '{ctx.node_id}': {target_kind} mask is empty."
        )
...
def _read_cfg(spec: dict, node_id: str) -> Config:
    ...
    if target not in SUBJECT_CROP2_TARGETS:
        supported = ', '.join(repr(t) for t in sorted(SUBJECT_CROP2_TARGETS))
        raise ValueError(
            f"'{node_id}': target={target!r} is not implemented by "
            f"SubjectCrop2 (supported: {supported})"
        )
    parsed_target = parse_target_specs(target, node_id=node_id)
    if target in SUBJECT_CROP2_POSE_TARGETS and pose_landmarker_task is None:
        raise ValueError(
...
def _predict_search_segments(
    ...
    *,
    ...
    if x2 <= x1 or y2 <= y1:
        raise RuntimeError(
            f"SubjectCrop2 node '{node_id}': invalid search bbox={bbox!r}."
        )
...
def _predict_pose(
    ...
    *,
    ...
    if x2 <= x1 or y2 <= y1:
        raise RuntimeError(
            f"SubjectCrop2 node '{node_id}': invalid pose bbox={bbox!r}."
        )
    ...
    if not np.any(valid):
        raise RuntimeError(
            f"SubjectCrop2 node '{node_id}': MediaPipe returned no usable "
            'pose landmarks.'
        )
...
def _target_requires_pose(target: str) -> bool:
    return target in SUBJECT_CROP2_POSE_TARGETS
...
def _require_pose(
    ...
    if ctx.pose is None:
        raise RuntimeError(
            f"SubjectCrop2 node '{ctx.node_id}': "
...
def _resolve_semantic_region(
    ...
    *,
    ...
    if not np.any(mask):
        raise RuntimeError(
            f"SubjectCrop2 node '{ctx.node_id}': semantic region is empty "
...
def _select_limb_side_by_proximal_landmark(
    *,
    ...
    if not candidates:
        raise RuntimeError(
            f"SubjectCrop2 node '{ctx.node_id}': no valid proximal "
...
def _segment_simple_target(
    ...
    if not np.any(selected_mask):
        raise RuntimeError(
            f"SubjectCrop2 node '{ctx.node_id}': structural cleanup removed "
...
class SubjectCrop2(CudaPostRunMixin, NodeRef):
    ...
    handled by ``FaceCrop``. ``SubjectCrop2`` is responsible for person
